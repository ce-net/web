/**
 * Map SDK / network errors to a friendly, actionable UI message plus a coarse category
 * the UI uses to pick an affordance. We narrow on the SDK's error classes by their
 * `name` (works even if two SDK copies are bundled) and fall back to duck-typing on
 * `status`.
 */
...
export function toFriendly(e: unknown): FriendlyError {
  ...
  if (name === "CeAuthError" || status === 401 || status === 403) {
    return {
      kind: "auth",
      message: "Your node rejected that write.",
      hint: "Set CE_API_TOKEN for the node (or run the node with auth off) to publish board ops.",
    };
  }
  if (name === "CeInsufficientFundsError" || status === 402) {
    return { kind: "funds", message: "Not enough credits for that action.", hint: "Mine or receive credits, then retry." };
  }
  if (name === "CeNotFoundError" || status === 404) {
    return { kind: "notfound", message: "Snapshot not found on this node.", hint: "It may not have propagated yet — tailing live ops instead." };
  }
  if (name === "CeRateLimitError" || status === 429) {
    ...
    return { kind: "ratelimit", message: "Publishing too fast.", hint: ra ? `Wait ${ra}s and retry.` : "Slow down and retry." };
  }
